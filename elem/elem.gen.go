...
// post, a magazine or newspaper article, a blog entry, an object, or any
...
// styling, using CSS, may place it elsewhere, relative to the table.
//
...
// within a dialog by specifying them with the attribute method="dialog". When
// such a form is submitted, the dialog is closed with a returnValue attribute
// set to the value of the submit button used.
//
...
// logo, wrapped section's header, a search form, and so on.
//
...
// heading of a section. It defines a single title that participates in the
// outline of the document as the heading of the implicit or explicit section
// that it belongs to.
//
...
// is now defined in semantic terms, rather than presentational terms.
//
...
// The HTML <param> Element (or HTML Parameter Element) defines parameters for
// <object>.
//
...
// elements for a specific <img> contained in it. The browser will choose the
// most suitable source according to the current layout of the page (the
// constraints of the box the image will appear in) and the device it will be
// displayed on (e.g. a normal or hiDPI device.)
//
...
// The HTML <progress> Element is used to view the completion progress of a
// task. While the specifics of how it's displayed is left up to the browser
// developer, it's typically displayed as a progress bar. Javascript can be
// used to manipulate the value of progress bar.
//
...
// The HTML <shadow> element is used as a shadow DOM insertion point. You
// might use it if you have created multiple shadow roots under a shadow host.
// It is not useful in ordinary HTML. It is used with Web Components.
//
...
// elements are immediate children of that table element.)  In conjunction
// with a preceding <thead> and/or <tfoot> element, <tbody> provides additional
...
// <thead>, <tfoot>, and <caption> elements of the same parent <table>
// element.  Note that unlike the <thead>, <tfoot>, and <caption> elements
// however, multiple <tbody> elements are permitted (if consecutive), allowing
// the data-rows in long tables to be divided into different sections, each
...
// The HTML element table row <tr> defines a row of cells in a table. Those
// can be a mix of <td> and <th> elements.
//
...
// automatically( just like the fixed sider bar of this page on the right).
//
